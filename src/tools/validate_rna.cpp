 ...
 * @brief Use blat alignment information to validate the correctness of contigs.
...
int main(int argc, char *argv[])
{
    ...
    vector<string> good_contigs(refs.size());
    ...
    string blat_file = string(argv[2]) + ".rna.blat";
    ...
    while (fgets(line, MaxLine, fblat) != NULL)
    {
           ...
           )
        {
            ...
            {
                ++aligned_times;
                if (is_found[ref_id] == false)
                {
                    string tmp = record.query_name;
                    Replace(tmp, '_', ' ');
                    int size = 0;
                ...
                good_contigs[ref_id] = record.query_name;
            }
            else
                continue;
...
//            for (int i = max(0, (int)record.ref_from -min_contig); i < min(record.ref_length, record.ref_to + min_contig); ++i)
//                flags[ref_id][i] = true;
...
//            valid_contigs.insert(record.query_name);
    ...
    printf("ratio: %.2f%% %d %d\n", 100.0 * aligned_times / found, aligned_times, found);
...
//    }
    ...
    if (flist != NULL)
    {
        deque<double> coverage(refs.size());
        for (unsigned i = 0; i < coverage.size(); ++i)
            fscanf(flist, "%s %lf", buf, &coverage[i]);

        FILE *fout = OpenFile(argv[2] + string(".list"), "wb");
        for (unsigned i = 0; i < coverage.size(); ++i)
        {
            if (is_found[i])
            {
                fprintf(fout, "%s %.4f %s\n", ref_names[i].c_str(), coverage[i], good_contigs[i].c_str());
            }
        }
    }
